...
const HeaderNav = () => {
  ...
  const { categories, isCategoriesLoading, categoriesError } = useFetchCategories();
  ...
  return (
    <nav className="flex w-full justify-center items-center px-6 py-3 bg-primary-card lg:bg-primary-bg">
      <div className="w-full max-w-[1400px] flex items-center justify-evenly">
        <Logo />
        {menu && <div onClick={onShadowClick} className="fixed bg-black w-full h-screen bg-opacity-30 top-0 left-0 z-40" />}
        <div className={`
        ...
        >
          ...
          {menu &&
            <svg
              onClick={onMenuClick} className="fill-primary-bg hover:cursor-pointer fixed right-3 top-3 m-3 z-50 lg:hidden" width="33" height="33" viewBox="0 0 33 33" fill="none"
        ...
        <svg onClick={onMenuClick} className="fill-primary-bg hover:cursor-pointer lg:hidden" width="48" height="32" viewBox="0 0 48 32"
          xmlns="http://www.w3.org/2000/svg">
